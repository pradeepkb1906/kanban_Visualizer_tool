...
> **CRITICAL — tool call is mandatory:** You MUST call `render_visualization(title="…")` as a function call BEFORE every `@@@VIZ-START` block. Without the tool call, no iframe is mounted and the content displays as raw text. Never emit `@@@VIZ-START` in a response where you did not call `render_visualization` in that same response turn.
...
1. Call `render_visualization(title="…")` — **this is not optional; do it first, every time**
...
**Example response structure (tool call MUST precede the markers):**

```
[tool call: render_visualization(title="Attention mechanism")]
...
**Streaming rules:**
- **MANDATORY:** Call `render_visualization()` first in every response that uses `@@@VIZ-START`. No exceptions.
